...
export class HyperledgerFabricService {
  ...
  constructor() {
    this.config = {
      ...
      connectionProfilePath: process.env.FABRIC_CONNECTION_PROFILE || './server/blockchain/connection-profile.json',
  ...
  async initializeConnection(): Promise<void> {
    try {
      ...
      console.log('🚀 === HYPERLEDGER FABRIC READY FOR REAL TRANSACTIONS ===\n');

    } catch (error) {
      console.error('❌ Failed to initialize Hyperledger Fabric connection:', error);
      console.log('⚠️  Falling back to simulation mode until network is available');
  ...
  private async loadConnectionProfile(): Promise<any> {
    try {
      if (fs.existsSync(this.config.connectionProfilePath)) {
        const profileData = fs.readFileSync(this.config.connectionProfilePath, 'utf8');
  ...
  async submitKYC(kycData: any, documentHashes: string[]): Promise<{ success: boolean; txHash: string; error?: string }> {
    try {
      ...
      if (!this.isConnected || !this.contract) {
        console.log('⚠️  Fabric network not connected, generating simulation transaction');
  ...
  async updateKYCStatus(kycId: string, status: string, remarks: string, verifiedBy: string): Promise<{ success: boolean; txHash: string; error?: string }> {
    try {
  ...
  async queryKYC(kycId: string): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      console.log(`\n🔍 === REAL BLOCKCHAIN QUERY: GET KYC ===`);
      console.log(`📋 KYC ID: ${kycId}`);

      if (!this.isConnected || !this.contract) {
        console.log('⚠️  Fabric network not connected, cannot query blockchain');
  ...
  async getKYCHistory(kycId: string): Promise<{ success: boolean; data?: any[]; error?: string }> {
    try {
      ...
      const result = await this.contract.evaluateTransaction('getKYCHistory', kycId);
      const historyData = JSON.parse(result.toString());

      console.log(`✅ REAL BLOCKCHAIN SUCCESS: ${historyData.length} history entries retrieved`);
      console.log('📜 === BLOCKCHAIN QUERY COMPLETED ===\n');
  ...
  private generateSimulationTransaction(type: string): { success: boolean; txHash: string } {
  ...
  async getNetworkStatus(): Promise<{ connected: boolean; network: string; chaincode: string }> {
    return {
