...
export class IPFSService {
  ...
  async initializeConnection(): Promise<void> {
    try {
      ...
      console.log(
        "⚠️  IPFS connection failed, falling back to simulation mode",
      );
  ...
  async getFile(
    ...
    try {
      ...
      console.log(
        `📋 File Size: ${(fileData.length / 1024 / 1024).toFixed(2)} MB`,
      );
