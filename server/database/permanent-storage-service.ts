...
export class PermanentStorageService {
  ...
  private async processVerifiedRecords(): Promise<void> {
    try {
      console.log("🔍 Checking for verified records requiring permanent storage...");
      ...
      console.log(`📊 Found ${verifiedRecords.length} verified records for permanent storage`);
      ...
      if (verifiedRecords.length > 0) {
        console.log(`✅ Processed ${verifiedRecords.length} records for permanent storage`);
      }
    } catch (error) {
  ...
  private async ensurePermanentStorage(record: any): Promise<void> {
    try {
      ...
      if (!record.blockchainVerificationTx) {
        console.log(`⛓️ Creating permanent blockchain record for ${record.id}...`);
        ...
        if (permanentResult.success) {
          ...
          console.log(`⛓️ Permanent blockchain record created: ${permanentResult.txId}`);
        } else {
          console.warn(`⚠️ Failed to create permanent blockchain record for ${record.id}`);
        }
      }
      ...
      await prisma.auditLog.create({
        data: {
          ...
          remarks: 'Record processed for permanent storage and blockchain immutability',
        },
      });

      console.log(`✅ Permanent storage ensured for record ${record.id}`);
    } catch (error) {
      console.error(`❌ Error ensuring permanent storage for record ${record.id}:`, error);
    }
  }
  ...
  private async ensureDocumentPermanentStorage(document: any): Promise<void> {
    try {
      ...
      console.error(`❌ Error ensuring document permanent storage for ${document.id}:`, error);
    }
  }
  ...
  async processPermanentStorageNow(): Promise<{ processed: number; errors: number }> {
