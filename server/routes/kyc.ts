...
class MockBlockchainService {
  async submitKYC(kycData: any, documentHashes: string[]): Promise<BlockchainTxResponse> {
  ...
  async verifyKYC(kycId: string): Promise<{ verified: boolean; txHash?: string }> {
...
class MockIPFSService {
  async uploadFile(file: Buffer, filename: string): Promise<IPFSUploadResponse> {
...
export const handleKYCSubmission: RequestHandler = async (req, res) => {
  try {
    ...
    const blockchainResult = await blockchainService.submitKYC(validatedData, documentHashes);
    ...
    const response: ApiResponse = {
      success: false,
      message: error instanceof z.ZodError ? error.errors[0].message : "Submission failed",
